#!/bin/bash
# ═══════════════════════════════════════════════════════════════════
# (C) Copyright 2024 Oleh Java Tunneling & jabarputra
# ═══════════════════════════════════════════════════════════════════
# Nama        : Autoscript Java Tunneling
# Info        : Memasang Berbagai jenis servis vpn didalam satu script
# Dibuat Pada : 01-08-2024 ( ........ )
# OS Support  : Ubuntu & Debian
# Owner       : Java Tunneling & jabarputra
...
clear;clear;clear
  ...
  # // Banner
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo -e "  Welcome To Java Tunneling Autoscript ${YELLOW}(${NC}${green} Stable Edition ${NC}${YELLOW})${NC}"
echo -e "     This Will Quick Setup VPN Server On Your Server"
echo -e "         Auther : ${green}JabarVpn ${NC}${YELLOW}(${NC} ${green}Jabar Project ${NC}${YELLOW})${NC}"
echo -e "       © JabarVpn & Jabar Project ${YELLOW}(${NC} 2023 ${YELLOW})${NC}"
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo ""
sleep 2
###### IZIN SC 
...
checking_sc() {
  ...
  if [[ $(date -d "$date_list" +%s) -lt $(date -d "$useexp" +%s) ]]; then
    ...
    echo -e "\033[42m          JAVA TUNNELING AUTOSCRIPT          \033[0m"
    echo -e "\033[1;93m────────────────────────────────────────────\033[0m"
    echo -e ""
    ...
    echo -e "      ${GREEN}WhatsApp${NC} wa.me/6283835753089"
    echo -e "\033[1;93m────────────────────────────────────────────\033[0m"
    exit
  fi
}
...
# Status ExpiRED Active | JabarVpn
...
function notif_bot() {
    ...
    USRSC=$(curl https://raw.githubusercontent.com/izinkansaya/Jabarputra/main/ip | grep $ipsaya | awk '{print $2}')
    EXPSC=$(curl https://raw.githubusercontent.com/izinkansaya/Jabarputra/main/ip | grep $ipsaya | awk '{print $3}')
    TIMES=`date -d "0 days" +"%d-%m-%Y" `
    TIMEZONE=$(printf '%(%H:%M:%S)T')
    TEXT="
<code>───────────────────────</code>
<b>    AUTOSCRIPT JAVA TUNNELING   </b>
<code>───────────────────────</code>
